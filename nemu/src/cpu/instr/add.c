...
make_instr_func(add_i2eax_b)
{
    ...
    myeax.val = alu_add(imm.val, myeax.val, myeax.data_size);
    operand_write(&myeax);

    print_asm_2("add", "", 2, &imm, &myeax);
    return 2;
}
...
make_instr_func(add_i2eax_v)
{
    ...
    len += data_size / 8;
    print_asm_2("add", "", len + 1, &imm, &myeax);
    return len;
}
...
make_instr_func(add_si2rm_v)
{
	...
	rm.val = alu_add(tmp, rm.val, rm.data_size);
    operand_write(&rm);
